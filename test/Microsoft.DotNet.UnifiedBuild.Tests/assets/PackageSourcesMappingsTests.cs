...
namespace Microsoft.DotNet.UnifiedBuild.Tests
{
    ...
    public class PackageSourceMappingsTests
    {
        ...
        {
            private static PackageSourceMappingsSetup instance = null;
            ...
            private PackageSourceMappingsSetup()
            {
                ...
                GenerateNuGetPackage(ArcadeSource, "Arcade.Package.One", "1.0.0");
                GenerateNuGetPackage(ArcadeSource, "Arcade.Package.Two", "1.0.0");

                // Generate Runtime nuget packages
                GenerateNuGetPackage(RuntimeSource, "Runtime.Package.One", "1.0.0");
                GenerateNuGetPackage(RuntimeSource, "Runtime.Package.Two", "1.0.0");

                // Generate SBRP nuget packages
                GenerateNuGetPackage(SourceBuildReferencePackagesSource, "SBRP.Package.One", "1.0.0");
                GenerateNuGetPackage(SourceBuildReferencePackagesSource, "SBRP.Package.Two", "1.0.0");

                // Generate previously-source-built packages
                GenerateNuGetPackage(PreviouslySourceBuiltSource, "PSB.Package.One", "1.0.0");
                GenerateNuGetPackage(PreviouslySourceBuiltSource, "PSB.Package.Two", "1.0.0");

                // Generate reference packages
                GenerateNuGetPackage(ReferencePackagesSource, "Reference.Package.One", "1.0.0");
                GenerateNuGetPackage(ReferencePackagesSource, "Reference.Package.Two", "1.0.0");

                // Generate prebuilt packages
                GenerateNuGetPackage(PrebuiltSource, "Prebuilt.Package", "1.0.0");

                // Generate SBRP repo files - nuspecs
                GenerateNuspecFile(SourceBuildReferencePackagesRepoDir, "SBRP.Repo.Package.One", "1.0.0");
                GenerateNuspecFile(SourceBuildReferencePackagesRepoDir, "SBRP.Repo.Package.Two", "1.0.0");
                GenerateNuspecFile(SourceBuildReferencePackagesRepoDir, "SBRP.Repo.Package.Three", "1.0.0");
                GenerateNuspecFile(SourceBuildReferencePackagesRepoDir, "SBRP.Repo.Package.Four", "1.0.0");
            }
